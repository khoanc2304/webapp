IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = '8386shop_new')
BEGIN
    CREATE DATABASE [8386shop_new];
END
GO
...
BEGIN
    CREATE TABLE Users (
        ...
        userUpdatedDate DATETIME DEFAULT GETDATE(),
        userStatus VARCHAR(20) DEFAULT 'active' CHECK (userStatus IN ('active', 'suspended')),
        userRole VARCHAR(20) DEFAULT 'customer' CHECK (userRole IN ('admin', 'manager', 'employee', 'customer'))
    );
END
GO
...
BEGIN
    CREATE TABLE Product (
        ...
        Description TEXT,
        ImageURL VARCHAR(255),
        ProductCreatedDate DATETIME DEFAULT GETDATE(),
        ProductUpdatedDate DATETIME DEFAULT GETDATE(),
        FOREIGN KEY (BrandID) REFERENCES Brand(BrandID) ON DELETE CASCADE
    );
END
GO

-- Thêm cột productStatus vào bảng Product
IF NOT EXISTS (SELECT * FROM sys.columns WHERE name = 'productStatus' AND object_id = OBJECT_ID('Product'))
BEGIN
    ALTER TABLE Product
    ADD productStatus VARCHAR(20) DEFAULT 'available' CHECK (productStatus IN ('available', 'unavailable'));
END
GO

-- Bảng ProductDetail: Chi tiết thuộc tính của sản phẩm
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'ProductDetail')
BEGIN
...
BEGIN
    CREATE TABLE Orders (
        ...
        orderStatus VARCHAR(20) DEFAULT 'Pending' CHECK (orderStatus IN ('Pending', 'Processing', 'Completed', 'Cancelled')),
...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'ProductReview')
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'ProductSold')
BEGIN
